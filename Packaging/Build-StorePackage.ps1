...
$results = foreach ($package in $packages) {
    ...
    try {
        ...
        $calibreBundled = $null -ne ($zip.Entries | Where-Object { $_.FullName -match '(?i)(^|/|\\)calibre|ebook-convert\.exe$' } | Select-Object -First 1)
        [pscustomobject]@{
            ...
            ConversionRuntimeDetached = (-not $isMainPackage) -or (-not $calibreBundled)
        }
    } finally { $zip.Dispose() }
}
if ($results.NameValid -contains $false -or $results.PublisherValid -contains $false -or $results.LanguagesValid -contains $false -or $results.ConversionRuntimeDetached -contains $false) {
    $results | Format-Table | Out-String | Write-Error
    throw 'Store package identity, language, or detached-runtime validation failed.'
}
$metadata = [ordered]@{
