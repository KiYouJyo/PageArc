...
public sealed class V13DataManagementParityTests
{
    ...
    [Fact]
    public void V13VersionContract_IsConsistent()
    {
        var root = FindRepoRoot();
        ...
        Assert.Contains("<Version>1.3.0</Version>", project, StringComparison.Ordinal);
        Assert.Contains("<AssemblyVersion>1.3.0.0</AssemblyVersion>", project, StringComparison.Ordinal);
        Assert.Contains("<ApplicationDisplayVersion>1.3</ApplicationDisplayVersion>", project, StringComparison.Ordinal);
        Assert.Contains("Version=\"1.3.0.0\"", githubManifest, StringComparison.Ordinal);
