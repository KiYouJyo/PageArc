...
public sealed class SettingsDataCompletionTests
{
    ...
    public async Task ReadingBackup_RoundTripsBookmarksAnnotationsAndProgress()
    {
        ...
        try
        {
            ...
            {
                BookId = "book-1",
                Locator = new FlowContentLocator(2, 0.4, TextQuote: "Highlighted quote"),
            ...
            var books = new[]
            {
                new BookEntry
                {
                    ...
                    SpineIndex = 2,
                    SectionFraction = 0.4,
                    LastOpenedAt = new DateTimeOffset(2026, 8, 17, 1, 2, 3, TimeSpan.Zero)
                }
            };
    ...
    public void CacheClear_RemovesGeneratedFilesButKeepsExternalDataAndResetsCachedCoverPaths()
    {
        ...
        File.WriteAllText(cachedCover, "cache");
        File.WriteAllText(externalCover, "external");
        File.WriteAllText(data, "user-data");

        try
        {
            var cachedBook = new BookEntry { CoverPath = cachedCover };
            var externalBook = new BookEntry { CoverPath = externalCover };
            var changed = CacheMaintenanceService.ClearGeneratedCache(cache, new[] { cachedBook, externalBook });

            Assert.Equal(1, changed);
            Assert.Null(cachedBook.CoverPath);
            Assert.Equal(externalCover, externalBook.CoverPath);
            Assert.True(File.Exists(data));
            Assert.Equal("user-data", File.ReadAllText(data));
            Assert.True(Directory.Exists(cache));
            Assert.False(File.Exists(cachedCover));
        }
        finally
        {
            if (Directory.Exists(root)) Directory.Delete(root, true);
    ...
    public void Categories_PersistNewCategoryAndRecoverBookAssignedCategory()
    {
        ...
        var file = Path.Combine(root, "categories.json");
        Directory.CreateDirectory(root);

        try
        {
            var categories = new CategoryService(file);
            categories.Load([]);
            categories.Add("Research");

            var reloaded = new CategoryService(file);
            reloaded.Load(new[] { new BookEntry { Collection = "Design" } });

            Assert.Contains(reloaded.Categories, x => x.Name == "Research");
            Assert.Contains(reloaded.Categories, x => x.Name == "Design");
        }
        finally
        {
            if (Directory.Exists(root)) Directory.Delete(root, true);
